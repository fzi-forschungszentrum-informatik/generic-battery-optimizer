...
class Test_Generate_Common_Time_SeriesDeprecated:
    """
...
class Test_Adjust_Battery_Timestamps:
    ...
    def test_forward_adjustment(self):
        ...
        with pytest.deprecated_call():
            assert (
                adjust_battery_timestamps(battery, index) == adjusted_battery
            )
...
class Test_Mixed_Timestamp_Adjustments:
    ...
    def test_profiles_batteries_heat_pumps(self):
        ...
        # battery
        with pytest.deprecated_call():
            assert adjust_battery_timestamps(
                battery, index
            ) == battery.model_copy(
                update={
                    "start_soc_time": pd.Timestamp("2024-01-01 01:30+00:00"),
        ...
        with pytest.deprecated_call():
            assert (
                adjust_battery_timestamps(battery, index).start_soc_time
                in index
            )
        with pytest.deprecated_call():
            assert (
                adjust_battery_timestamps(battery, index).end_soc_time in index
            )
